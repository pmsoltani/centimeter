...
`Account` has no field naming an element, and `RootAccounts` holds exactly five IDs, one per element, so a sixth root cannot be constructed. A test asserts that an account's element is resolved by walking to its root and that a reparented subtree changes element accordingly.
