...
impl CommodityRegistry {
    ...
    // TODO: add methods for len, iter, update, etc. as needed.
    // NOTE: removing commodities may be tricky if they could be referenced by
    // posted transactions.
}
...
mod tests {
    ...
    fn test_codes_are_case_sensitive() {
        ...
        assert_eq!(reg.get_by_code("USD").unwrap().id(), id(1));
